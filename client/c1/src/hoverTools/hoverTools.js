...
function onCopyBtnClicked() {
  ...
  navigator.clipboard.writeText(highlightText).then(result => result);
}
...
function onDeleteBtnClicked() {
  ...
  highlights.each((_, el) => {
    el.replaceWith(el.cloneNode(true)); // Replace the highlight with its clone
  });
}
...
function onChangeColorBtnClicked() {
  ...
  const newColor = colors[(colors.indexOf(currentColor) + 1) % 3];
  highlights.css('backgroundColor', newColor); // Change the background color attribute

  update(highlightId, window.location.href, window.location.pathname, newColor); // update the value in the local storage
}
...
$.get(chrome.extension.getURL('hoverTools/hoverTools.html'), data => {
  ...
  shareBtnEl = hoverToolEl.find('.highlighter--icon-share')[0];
